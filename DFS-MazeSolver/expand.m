...
function exp_array = expand(node_x, node_y, OBSTACLE, MAX_X, MAX_Y)
    exp_array = []; % children nodes
    exp_count = 1;
    c2 = size(OBSTACLE, 1);
    for k = [1,0,-1] % explore surrounding locations by order
        for j = [1,0,-1]
            if (k * k + j * j == 1)  % the node itself is not its successor%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                s_x = node_x + k;
                s_y = node_y + j;
                if((s_x > 0 && s_x <= MAX_X) && (s_y > 0 && s_y <= MAX_Y)) % node within array bound5555555555555555555555555555555555555555555555555
                    flag = 1;                    
                    for c1 = 1 : c2
